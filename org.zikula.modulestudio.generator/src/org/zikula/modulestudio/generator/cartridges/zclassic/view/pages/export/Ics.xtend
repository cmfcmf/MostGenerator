...
class Ics {
    ...
    def private icsDisplay(Entity it, String appName) '''
        ...
        IF container.application.targets('1.3.5')
            {appName.formatForDBTemplateHeaders contentType='text/calendar; charset=iso-8859-15'}{*charset=utf-8'*}
        ENDIF
        {php}
            $objName = $this->get_template_vars('objName');
            IF hasSluggableFields
                header('Content-Disposition: attachment; filename="name.formatForCode_' . $objName['slug'] . '.ics"');
            ELSE
                header('Content-Disposition: attachment; filename="name.formatForCode_' . $objName->getTitleFromDisplayPattern() . '.ics"');
            ENDIF
        {/php}
        BEGIN:VCALENDAR
        VERSION:2.0
        ...
        {*if $objName.zipcode ne '' && $objName.city ne ''}{assign var='location' value="`$objName.zipcode` `$objName.city`"}LOCATION{$location|appName.formatForDBFormatIcalText}{/if}
        IF geographical
        ...
        ENDIF
        SUMMARY{$objName->getTitleFromDisplayPattern()|appName.formatForDBFormatIcalText}
        IF hasTextFieldsEntity
            val field = getTextFieldsEntity.head
            {if $objName.field.name.formatForCode ne ''}DESCRIPTION{$objName.field.name.formatForCode|appName.formatForDBFormatIcalText}{/if}
        ENDIF
        PRIORITY:5
        IF hasUploadFieldsEntity
            FOR field : getUploadFieldsEntity
                {if $objName.field.name.formatForCode}ATTACH;VALUE=URL:{$objName.field.name.formatForCodeFullPathURL}
                {/if}
            ENDFOR
        ENDIF
