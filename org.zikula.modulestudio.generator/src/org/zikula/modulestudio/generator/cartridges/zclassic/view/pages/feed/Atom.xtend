...
class Atom {
    ...
    def private atomView(Entity it, String appName) '''
        ...
        {/if}
        IF container.application.targets('1.3.5'){appName.formatForDBTemplateHeaders contentType='application/atom+xml'}ENDIF<?xml version="1.0" encoding="{charset assign='charset'}{if $charset eq 'ISO-8859-15'}ISO-8859-1{else}{$charset}{/if}" ?>
        ...
        {if $numItems}
        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:IF app.targets('1.3.5'){modurl modname='appName' type=$lct func='defaultAction' ot='name.formatForCode'IF hasActions('display') routeParamsLegacy('items[0]', true, true)ENDIF}ELSE{route name='appName.formatForDB_name.formatForCode_IF hasActions('display')displayELSEIF hasActions('view')viewELSEindexENDIFENDIF'IF hasActions('display') routeParams('items[0]', true)ENDIF lct=$lct}ENDIF{/capture}
            <id>{$uniqueID}</id>
            <updated>{$items[0].updatedDate|default:$smarty.now|dateformat:'%Y-%m-%dT%H:%M:%SZ'}</updated>
        {/if}
        IF app.targets('1.3.5')
            <link rel="alternate" type="text/html" hreflang="{lang}" href="{modurl modname='appName' type=$lct func='IF hasActions('index')mainELSEIF hasActions('view')view' ot='name.formatForCodeELSEapp.getAdminAndUserControllers.map[actions].flatten.toList.head.name.formatForCodeENDIF' fqurl=true}" />
        ELSE
            <link rel="alternate" type="text/html" hreflang="{lang}" href="{route name='appName.formatForDB_name.formatForCode_IF hasActions('index')indexELSEIF hasActions('view')view' lct=$lctELSEapp.getAdminAndUserControllers.map[actions].flatten.toList.head.name.formatForCodeENDIF' absolute=true}" />
        ENDIF
        ...
        {foreach item='objName' from=$items}
            <entry>
                ...
                ELSE
                    <link rel="alternate" type="text/html" href="{route name='appName.formatForDB_name.formatForCode_defaultAction'IF hasActions('display') routeParams(objName, true)ENDIF lct=$lct absolute=true}" />
                    {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$objName.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{route name='appName.formatForDB_name.formatForCode_defaultAction'IF hasActions('display') routeParams(objName, true)ENDIF lct=$lct}{/capture}
                ENDIF
                <id>{$uniqueID}</id>
                IF standardFields
