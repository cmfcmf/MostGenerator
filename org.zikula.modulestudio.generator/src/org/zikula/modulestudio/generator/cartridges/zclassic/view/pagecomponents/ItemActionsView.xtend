...
class ItemActionsView {
    ...
    def private markup(Entity it, String context) '''
        ...
        ELSE
            ...
            ENDIF
            <div class="dropdown">
                <ul class="dropdown-menu" role="menu" aria-labelledby="itemActionContainerViewIdDropDownToggle">
                    linkList(context)
                </ul>
            </div>
        ENDIF
...
*/
    ...
    def private linkEntryCommonAttributes(Entity it) '''href="{$option.url.type|container.application.appName.formatForDBActionUrl:$option.url.func:$option.url.arguments}" title="{$option.linkTitle|safetext}"'''
    ...
    def trigger(Entity it, String context) '''
        IF container.application.targets('1.3.5')
            {icon id="itemActionContainerViewIdForSmartyTrigger" type='options' size='extrasmall' __alt='Actions' class='z-pointer z-hide'}
        ELSE
            <a id="itemActionContainerViewIdDropDownToggle" role="button" data-toggle="dropdown" data-target="#" href="javascript:void(0);" class="dropdown-toggle"><i class="fa fa-tasks"></i>IF context == 'display' {gt text='Actions'}ENDIF <span class="caret"></span></a>
/*            <button id="itemActionContainerViewIdDropDownToggle" class="btn btn-default dropdown-toggle" type="button" data-toggle="dropdown"><i class="fa fa-tasks"></i>IF context == 'display' {gt text='Actions'}ENDIF <span class="caret"></span></button>*/
        ENDIF
    '''

    def itemActionContainerViewId(Entity it) '''
        itemActionsFOR pkField : getPrimaryKeyFields SEPARATOR '_'{$name.formatForCode.pkField.name.formatForCode}ENDFOR'''
