...
class Delete {
    ...
    def private deleteView(Entity it, String appName) '''
        ...
        <div class="appName.toLowerCase-name.formatForDB appName.toLowerCase-delete">
            ...
            <p class="IF app.targets('1.3.5')z-warningmsgELSEalert alert-warningmsgENDIF">{gt text='Do you really want to delete this name.formatForDisplay ?'}</p>

            <form class="IF app.targets('1.3.5')z-formELSEform-horizontalENDIF" action="IF app.targets('1.3.5'){modurl modname='appName' type=$lct func='delete' ot='name.formatForCode' routeParamsLegacy(name, true, false)}ELSE{route name='appName.formatForDB_name.formatForCode_delete' routeParams(name, true) lct=$lct}ENDIF" method="post"IF !app.targets('1.3.5') role="form"ENDIF>
                <div>
                    <input type="hidden" name="csrftoken" value="{insert name='csrftoken'}" />
                    <input type="hidden" id="confirmation" name="confirmation" value="1" />
                    <fieldset>
                        <legend>{gt text='Confirmation prompt'}</legend>
                        <div class="IF app.targets('1.3.5')z-buttons z-formbuttonsELSEform-group form-buttonsENDIF">
                        IF !app.targets('1.3.5')
                            <div class="col-lg-offset-3 col-lg-9">
                        ENDIF
                            {gt text='Delete' assign='deleteTitle'}
                            {button src='14_layer_deletelayer.png' set='icons/small' text=$deleteTitle title=$deleteTitle class='IF app.targets('1.3.5')z-btredELSEbtn btn-dangerENDIF'}
                            IF app.targets('1.3.5')
                            ...
                            ELSE
                                <a href="{route name='appName.formatForDB_name.formatForCode_view' lct=$lct}" class="btn btn-default" role="button"><span class="fa fa-times"></span> {gt text='Cancel'}</a>
                            ENDIF
                        IF !app.targets('1.3.5')
                            </div>
                        ENDIF
