...
class Relations {
    ...
    def private includeStatementForEditTemplate(JoinRelationship it, String templateName, Entity ownEntity, Entity linkingEntity, Boolean incoming, String relationAliasName, String relationAliasReverse, String uniqueNameForJs, Boolean hasEdit) '''
        {include file='IF container.application.targets('1.3.5')ownEntity.name.formatForCodeELSEownEntity.name.formatForCodeCapitalENDIF/templateName.tpl' group='linkingEntity.name.formatForDB' alias='relationAliasName.toFirstLower' aliasReverse='relationAliasReverse.toFirstLower' mandatory=(!nullable).displayBool idPrefix='uniqueNameForJs' linkingItem=$linkingEntity.name.formatForDBIF linkingEntity.useGroupingPanels('edit') panel=trueENDIF displayMode='IF usesAutoCompletion(incoming)autocompleteELSEdropdownENDIF' allowEditing=hasEdit.displayBool}
    '''
    ...
    def private includedEditTemplate(JoinRelationship it, Application app, Entity ownEntity, Entity linkingEntity, Boolean incoming, Boolean hasEdit, Boolean many) '''
        ...
        {if isset($panel) && $panel eq true}
            IF app.targets('1.3.5')
                <h3 class="ownEntityName.formatForDB z-panel-header z-panel-indicator IF app.targets('1.3.5')zELSEcursorENDIF-pointer">{gt text='ownEntityName.formatForDisplayCapital'}</h3>
                <fieldset class="ownEntityName.formatForDB z-panel-content" style="display: none">
            ELSE
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title"><a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#collapseownEntityName.formatForCodeCapital">{gt text='ownEntityName.formatForDisplayCapital'}</a></h3>
                    </div>
                    <div id="collapseownEntityName.formatForCodeCapital" class="panel-collapse collapse in">
    ...
    def private includedEditTemplateBody(JoinRelationship it, Application app, Entity ownEntity, Entity linkingEntity, Boolean incoming, Boolean hasEdit, Boolean many) '''
        val ownEntityName = ownEntity.getEntityNameSingularPlural(many)
        <div class="IF container.application.targets('1.3.5')z-formrowELSEform-groupENDIF">
        ...
        {if $displayMode eq 'dropdown'}
            {formlabel for=$alias __text='Choose ownEntityName.formatForDisplay'IF !nullable mandatorysym='1'ENDIFIF !app.targets('1.3.5') cssClass='col-lg-3 control-label'ENDIF}
            IF !app.targets('1.3.5')
                <div class="col-lg-9">
            ENDIF
                {appnameLowerRelationSelectorList pluginAttributesIF !container.application.targets('1.3.5') cssClass='form-control'ENDIF}
            IF !app.targets('1.3.5')
                </div>
            ENDIF
        {elseif $displayMode eq 'autocomplete'}
            ...
            ELSE
                ...
                {if $allowEditing eq true}
                    ...
                    ELSE
                        {route name='app.appName.formatForDB_ownEntity.name.formatForCode_edit' lct=$lct assign='createLink'}
                    ENDIF
                {/if}
                {appnameLowerRelationSelectorAutoComplete pluginAttributes idPrefix=$idPrefix createLink=$createLink withImage=ownEntity.hasImageFieldsEntity.displayBoolIF !container.application.targets('1.3.5') cssClass='form-control'ENDIF}
    ...
    def private component_AutoComplete(JoinRelationship it, Application app, Entity targetEntity, Boolean many, Boolean incoming, Boolean includeEditing) '''
        <div class="app.appName.toLowerCase-relation-leftside">
            ...
            {if isset($linkingItem.$alias)}
                {includeStatement itemIF manysENDIF=$linkingItem.$alias}
            {else}
                {includeStatement}
            {/if}
        </div>
        <br IF app.targets('1.3.5')class="z-clearer"ELSEstyle="clear: both"ENDIF />
    '''
    ...
    def private component_IncludeStatementForAutoCompleterItemList(JoinRelationship it, Entity targetEntity, Boolean many, Boolean incoming, Boolean includeEditing) '''
        include file='IF container.application.targets('1.3.5')targetEntity.name.formatForCodeELSEtargetEntity.name.formatForCodeCapitalENDIF/include_selectIF includeEditingEditENDIFItemListIF !manyOneELSEManyENDIF.tpl' '''
    ...
    def private component_ItemList(JoinRelationship it, Application app, Entity targetEntity, Boolean many, Boolean incoming, Boolean includeEditing) '''
        ...
        <input type="hidden" id="{$idPrefix}ItemList" name="{$idPrefix}ItemList" value="{if isset($itemIF manysENDIF) && (is_array($itemIF manysENDIF) || is_object($itemIF manysENDIF))IF !manyFOR pkField : targetEntity.getPrimaryKeyFields && isset($item.pkField.name.formatForCode)ENDFORENDIF}IF many{foreach name='relLoop' item='item' from=$items}ENDIFFOR pkField : targetEntity.getPrimaryKeyFields SEPARATOR '_'{$item.pkField.name.formatForCode}ENDFORIF many{if $smarty.foreach.relLoop.last ne true},{/if}{/foreach}ENDIF{/if}" />
        <input type="hidden" id="{$idPrefix}Mode" name="{$idPrefix}Mode" value="IF includeEditing1ELSE0ENDIF" />

        <ul id="{$idPrefix}ReferenceList">
        {if isset($itemIF manysENDIF) && (is_array($itemIF manysENDIF) || is_object($itemIF manysENDIF))IF !manyFOR pkField : targetEntity.getPrimaryKeyFields && isset($item.pkField.name.formatForCode)ENDFORENDIF}
        IF many
            {foreach name='relLoop' item='item' from=$items}
        ENDIF
        {assign var='idPrefixItem' value="`$idPrefix`Reference_FOR pkField : targetEntity.getPrimaryKeyFields SEPARATOR '_'`$item.pkField.name.formatForCode`ENDFOR"}
        <li id="{$idPrefixItem}">
            ...
            IF includeEditing
                ...
                ELSE
                    <a id="{$idPrefixItem}Edit" href="{route name='app.appName.formatForDB_targetEntity.name.formatForCode_edit' targetEntity.routeParams('item', true) lct=$lct}">{$editImage}</a>
                ENDIF
            ENDIF
            ...
            IF targetEntity.hasImageFieldsEntity
                ...
                {if $item.imageFieldName ne '' && isset($item.imageFieldNameFullPath) && $item.imageFieldNameMeta.isImage}
                    {thumb image=$item.imageFieldNameFullPath objectid="targetEntity.name.formatForCodeIF targetEntity.hasCompositeKeysFOR pkField : targetEntity.getPrimaryKeyFields-`$item.pkField.name.formatForCode`ENDFORELSE-`$item.targetEntity.primaryKeyFields.head.name.formatForCode`ENDIF" preset=$relationThumbPreset tag=true img_alt=$item->getTitleFromDisplayPattern()IF !container.application.targets('1.3.5') img_class='img-rounded'ENDIF}
                {/if}
            ENDIF
        </li>
        IF many
    ...
    def initJs(Entity it, Application app, Boolean insideLoader) '''
        ...
        IF !incomingJoins.empty || !outgoingJoins.empty
            ...
            ENDIF
            FOR relation : incomingJoinsrelation.initJs(app, it, true, insideLoader)ENDFOR
            FOR relation : outgoingJoinsrelation.initJs(app, it, false, insideLoader)ENDFOR
        ENDIF
    '''
    ...
    def private initJs(JoinRelationship it, Application app, Entity targetEntity, Boolean incoming, Boolean insideLoader) {
        ...
        if (!insideLoader) '''
            ...
            newItem.prefix = 'uniqueNameForJsSelectorDoNew';
