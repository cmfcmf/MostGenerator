...
class Csv {
    ...
    def private csvView(Entity it, String appName) '''
        ...
        IF container.application.targets('1.3.5')
            {appName.formatForDBTemplateHeaders contentType='text/comma-separated-values; charset=iso-8859-15' asAttachment=true filename='nameMultiple.formatForCodeCapital.csv'}
        ENDIF
        {strip}FOR field : getDisplayFields.filter[name != 'workflowState'] SEPARATOR ';'field.headerLineENDFORIF geographicalFOR geoFieldName : newArrayList('latitude', 'longitude');"{gt text='geoFieldName.formatForDisplayCapital'}"ENDFORENDIFIF softDeleteable;"{gt text='Deleted at'}"ENDIF;"{gt text='Workflow state'}"
        ...
        {strip}
            FOR field : getDisplayFields.filter[e|e.name != 'workflowState'] SEPARATOR ';'field.displayEntryENDFORIF geographicalFOR geoFieldName : newArrayList('latitude', 'longitude');"{$name.formatForCode.geoFieldName|appName.formatForDBFormatGeoData}"ENDFORENDIFIF softDeleteable;"{$item.deletedAt|dateformat:'datebrief'}"ENDIF;"{$item.workflowState|appName.formatForDBObjectState:false|lower}"
