...
class Rss {
    ...
    def private rssView(Entity it, String appName) '''
        ...
        {/if}
        IF container.application.targets('1.3.5'){appName.formatForDBTemplateHeaders contentType='application/rss+xml'}ENDIF<?xml version="1.0" encoding="{charset assign='charset'}{if $charset eq 'ISO-8859-15'}ISO-8859-1{else}{$charset}{/if}" ?>
        <rss version="2.0"
        ...
        {foreach item='objName' from=$items}
            <item>
                ...
                ELSE
                    <link>{route name='appName.formatForDB_name.formatForCode_defaultAction'IF hasActions('display') routeParams(objName, true)ENDIF lct=$lct absolute=true}</link>
                    <guid>{route name='appName.formatForDB_name.formatForCode_defaultAction'IF hasActions('display') routeParams(objName, true)ENDIF lct=$lct absolute=true}</guid>
                ENDIF
                IF !standardFields
                    IF metaData
