...
class Validation {
    ...
    def private generate(Application it) '''
        ...
        IF getAllEntities.exists[getUniqueDerivedFields.filter[!primaryKey].size > 0]
            ...
            {
                var paramsIF targets('1.3.5'), requestENDIF;
                ...
                params = {
                    ot: ucOt,
                    fn: encodeURIComponent(elem.IF targets('1.3.5')idELSEattr('id')ENDIF),
                ...
                ELSE
                    $.ajax({
                        type: 'POST',
                        url: Routing.generate('appName.formatForDB_ajax_checkForDuplicate'),
                        data: params
                    }).done(function(res) {
        ...
        IF hasUploads
            ...
            function prefix()ValidateUploadExtension(val, elem)
            {
                ...
                allowedExtensions = $(IF !targets('1.3.5')'#' + elem.attr('id')ELSEelem.idENDIF + 'FileExtensions').innerHTML;
        ...
        FOR entity : getAllEntities
            ...
            IF startDateField !== null && endDateField !== null
                ...
                function prefix()ValidateDateRangeentity.name.formatForCodeCapital(val)
                {
                    ...
                    IF targets('1.3.5')
                        ...
                        if (result) {
                            ...
                            $('advice-validateClass-startFieldName').innerHTML = Zikula.__('The start must be before the end.', 'module_appName.formatForDB_js');
                            $('advice-validateClass-endFieldName').innerHTML = Zikula.__('The start must be before the end.', 'module_appName.formatForDB_js');
                    ...
                    ELSE
                        ...
                        if (result) {
                            ...
                            $('#advice-validateClass-startFieldName').html(Zikula.__('The start must be before the end.', 'module_appName.formatForDB_js'));
                            $('#advice-validateClass-endFieldName').html(Zikula.__('The start must be before the end.', 'module_appName.formatForDB_js'));
        ...
        /**
         * IF targets('1.3.5')AddsELSERunsENDIF special validation rules.
         */
        function prefix()IF targets('1.3.5')AddCommonValidationRulesELSEPerformCustomValidationRulesENDIF(objectType, id)
        {
            IF targets('1.3.5')
                Validation.addAllThese([
                    ['validate-nospace', Zikula.__('No spaces', 'module_appName.formatForDB_js'), function(val, elem) {
                        return prefix()ValidateNoSpace(val);
                    }],
                    IF hasColourFields
                        ['validate-htmlcolour', Zikula.__('Please select a valid html colour code.', 'module_appName.formatForDB_js'), function(val, elem) {
                            return prefix()ValidateHtmlColour(val);
                        }],
                    ENDIF
                    IF hasUploads
                        ['validate-upload', Zikula.__('Please select a valid file extension.', 'module_appName.formatForDB_js'), function(val, elem) {
                            return prefix()ValidateUploadExtension(val, elem);
                        }],
                    ENDIF
                    IF !datetimeFields.empty
                        IF datetimeFields.exists[past]
                            ['validate-datetime-past', Zikula.__('Please select a value in the past.', 'module_appName.formatForDB_js'), function(val, elem) {
                                return prefix()ValidateDatetimePast(val);
                            }],
                        ENDIF
                        IF datetimeFields.exists[future]
                            ['validate-datetime-future', Zikula.__('Please select a value in the future.', 'module_appName.formatForDB_js'), function(val, elem) {
                    ...
                    IF !dateFields.empty
                        IF dateFields.exists[past]
                            ['validate-date-past', Zikula.__('Please select a value in the past.', 'module_appName.formatForDB_js'), function(val, elem) {
                                return prefix()ValidateDatePast(val);
                            }],
                        ENDIF
                        IF dateFields.exists[future]
                            ['validate-date-future', Zikula.__('Please select a value in the future.', 'module_appName.formatForDB_js'), function(val, elem) {
                    ...
                    IF !timeFields.empty
                        IF timeFields.exists[past]
                            ['validate-time-past', Zikula.__('Please select a value in the past.', 'module_appName.formatForDB_js'), function(val, elem) {
                                return prefix()ValidateTimePast(val);
                            }],
                        ENDIF
                        IF timeFields.exists[future]
                            ['validate-time-future', Zikula.__('Please select a value in the future.', 'module_appName.formatForDB_js'), function(val, elem) {
                    ...
                    FOR entity : getAllEntities
                        ...
                        val endDateField = entity.getEndDateField
                        IF startDateField !== null && endDateField !== null
                            ['validate-daterange-entity.name.formatForDB', Zikula.__('The start must be before the end.', 'module_appName.formatForDB_js'), function(val, elem) {
                                return prefix()ValidateDateRangeentity.name.formatForCodeCapital(val);
                            }],
                        ENDIF
                    ENDFOR
                    IF getAllEntities.exists[getUniqueDerivedFields.filter[!primaryKey].size > 0]
                        ['validate-unique', Zikula.__('This value is already assigned, but must be unique. Please change it.', 'module_appName.formatForDB_js'), function(val, elem) {
            ...
            ELSE
                $('.validate-nospace').each( function() {
                    if (prefix()ValidateNoSpace($(this).val())) {
                        $(this).setCustomValidity(Zikula.__('This value must not contain spaces.', 'module_appName.formatForDB_js'));
                    } else {
                ...
                IF hasColourFields
                    $('.validate-htmlcolour').each( function() {
                        if (prefix()ValidateHtmlColour($(this).val())) {
                            $(this).setCustomValidity(Zikula.__('Please select a valid html colour code.', 'module_appName.formatForDB_js'));
                        } else {
                            $(this).setCustomValidity('');
                        }
                    });
                ...
                IF hasUploads
                    $('.validate-upload').each( function() {
                        if (prefix()ValidateUploadExtension($(this).val(), $(this))) {
                            $(this).setCustomValidity(Zikula.__('Please select a valid file extension.', 'module_appName.formatForDB_js'));
                        } else {
                            $(this).setCustomValidity('');
                        }
                    });
                    ...
                    IF datetimeFields.exists[past]
                        $('.validate-datetime-past').each( function() {
                            if (prefix()ValidateDatetimePast($(this).val())) {
                                $(this).setCustomValidity(Zikula.__('Please select a value in the past.', 'module_appName.formatForDB_js'));
                            } else {
                                $(this).setCustomValidity('');
                            }
                        });
                    ...
                    IF datetimeFields.exists[future]
                        $('.validate-datetime-future').each( function() {
                            if (prefix()ValidateDatetimeFuture($(this).val())) {
                                $(this).setCustomValidity(Zikula.__('Please select a value in the future.', 'module_appName.formatForDB_js'));
                            } else {
                                $(this).setCustomValidity('');
                            }
                        });
                ...
                FOR entity : getAllEntities
                    ...
                    IF startDateField !== null && endDateField !== null
                        $('.validate-daterange-entity.name.formatForDB').each( function() {
                            if (prefix()ValidateDateRangeentity.name.formatForCodeCapital($(this).val())) {
                                $(this).setCustomValidity(Zikula.__('The start must be before the end.', 'module_appName.formatForDB_js'));
                            } else {
                                $(this).setCustomValidity('');
                            }
                        });
                ...
                IF getAllEntities.exists[getUniqueDerivedFields.filter[!primaryKey].size > 0]
                    $('.validate-unique').each( function() {
                        if (prefix()UniqueCheck('name.formatForCode', $(this).val(), $(this), id)) {
                            $(this).setCustomValidity(Zikula.__('This value is already assigned, but must be unique. Please change it.', 'module_appName.formatForDB_js'));
                        } else {
                            $(this).setCustomValidity('');
                        }
                    });
