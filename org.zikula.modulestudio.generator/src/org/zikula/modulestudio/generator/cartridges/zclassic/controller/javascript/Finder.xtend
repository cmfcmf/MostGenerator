...
class Finder {
    ...
    def private generate(Application it) '''
        ...
        var currentappNameEditor = null;
        var currentappNameInput = null;
         ...
         */
        function appNameFinderXinha(editor, prefix()URL)
        {
            var popupAttributes;

            // Save editor for access in selector window
            currentappNameEditor = editor;
         ...
         */
        function appNameFinderCKEditor(editor, prefix()URL)
        {
            // Save editor for access in selector window
            currentappNameEditor = editor;
            ...
            editor.popup(
                ...
                ELSE
                    Routing.generate('appName.formatForDB_external_finder', { editor: 'ckeditor' }),
                ENDIF
                /*width*/ '80%', /*height*/ '70%',
        ...
        name.formatForDB.finder.onLoad = function (baseId, selectedId)
        {
            IF targets('1.3.5')
                ...
                $('appName.toFirstLowerSort').observe('change', name.formatForDB.finder.onParamChanged);
                $('appName.toFirstLowerSortDir').observe('change', name.formatForDB.finder.onParamChanged);
                $('appName.toFirstLowerPageSize').observe('change', name.formatForDB.finder.onParamChanged);
                $('appName.toFirstLowerSearchGo').observe('click', name.formatForDB.finder.onParamChanged);
                $('appName.toFirstLowerSearchGo').observe('keypress', name.formatForDB.finder.onParamChanged);
                $('appName.toFirstLowerSubmit').addClassName('z-hide');
                $('appName.toFirstLowerCancel').observe('click', name.formatForDB.finder.handleCancel);
            ELSE
                $('div.categoryselector select').change(name.formatForDB.finder.onParamChanged);
                $('#appName.toFirstLowerSort').change(name.formatForDB.finder.onParamChanged);
                $('#appName.toFirstLowerSortDir').change(name.formatForDB.finder.onParamChanged);
                $('#appName.toFirstLowerPageSize').change(name.formatForDB.finder.onParamChanged);
                $('#appName.toFirstLowerSearchGo').click(name.formatForDB.finder.onParamChanged);
                $('#appName.toFirstLowerSearchGo').keypress(name.formatForDB.finder.onParamChanged);
                $('#appName.toFirstLowerSubmit').addClass('hidden');
                $('#appName.toFirstLowerCancel').click(name.formatForDB.finder.handleCancel);
            ENDIF
        };

        name.formatForDB.finder.onParamChanged = function ()
        {
            $('IF !targets('1.3.5')#ENDIFappName.toFirstLowerSelectorForm').submit();
        };
        ...
        function getPasteSnippet(mode, itemId)
        {
            ...
            IF targets('1.3.5')
                ...
                pasteMode = $F('appName.toFirstLowerPasteAs');
            ELSE
                itemUrl = $('#url' + itemId).val();
                itemTitle = $('#title' + itemId).val();
                itemDescription = $('#desc' + itemId).val();
                pasteMode = $('#appName.toFirstLowerPasteAs').val();
            ENDIF
        ...
        name.formatForDB.finder.selectItem = function (itemId)
        {
            ...
            ENDIF
            if (editor === 'xinha') {
                if (window.opener.currentappNameEditor !== null) {
                    html = getPasteSnippet('html', itemId);

                    window.opener.currentappNameEditor.focusEditor();
                    window.opener.currentappNameEditor.insertHTML(html);
                } else {
                    html = getPasteSnippet('url', itemId);
                    var currentInput = window.opener.currentappNameInput;
        ...
        name.formatForDB.itemSelector.onLoad = function (baseId, selectedId)
        {
            ...
            IF targets('1.3.5')
                $('appName.toFirstLowerObjectType').observe('change', name.formatForDB.itemSelector.onParamChanged);
                ...
                $('appName.toFirstLowerSearchGo').observe('click', name.formatForDB.itemSelector.onParamChanged);
                $('appName.toFirstLowerSearchGo').observe('keypress', name.formatForDB.itemSelector.onParamChanged);
            ELSE
                $('#appName.toFirstLowerObjectType').change(name.formatForDB.itemSelector.onParamChanged);
                ...
                $('#appName.toFirstLowerSearchGo').click(name.formatForDB.itemSelector.onParamChanged);
                $('#appName.toFirstLowerSearchGo').keypress(name.formatForDB.itemSelector.onParamChanged);
            ENDIF
        ...
        {
            $('ajax_indicator').removeClassIF targets('1.3.5')NameENDIF('IF targets('1.3.5')z-hideELSEhiddenENDIF');
        ...
        {
            var baseId, paramsIF targets('1.3.5'), requestENDIF;
            ...
            ELSE
                $.ajax({
                    type: 'POST',
                    url: Routing.generate('appName.formatForDB_ajax_getItemListFinder'),
                    data: params
                }).done(function(res) {
        ...
        name.formatForDB.itemSelector.updateItemDropdownEntries = function ()
        {
            ...
            baseId = name.formatForDB.itemSelector.baseId;
            itemSelector = $(IF !targets('1.3.5')'#' + ENDIFbaseId + 'Id');
        ...
        name.formatForDB.itemSelector.onItemChanged = function ()
        {
            ...
            $(baseId + 'PreviewContainer').IF targets('1.3.5')updateELSEhtmlENDIF(preview);
