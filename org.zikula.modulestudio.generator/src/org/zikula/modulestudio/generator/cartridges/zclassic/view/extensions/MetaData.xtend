...
class MetaData {
    ...
    def private metaDataViewImpl(Application it) '''
        ...
        {if isset($obj.metadata)}
            {if isset($panel) && $panel eq true}
                IF targets('1.3.5')
                    <h3 class="metadata z-panel-header z-panel-indicator IF targets('1.3.5')zELSEcursorENDIF-pointer">{gt text='Metadata'}</h3>
    ...
    def private formRow(String fieldName, Integer length) '''
        ...
        <div class="IF useBootstrapform-groupELSEz-formrowENDIF">
            {formlabel for='metadatalabel' __text='label'IF useBootstrap cssClass='col-lg-3 control-label'ENDIF}
            IF useBootstrap
                <div class="col-lg-9">
            ENDIF
                IF 'startdate'.equals(fieldName) || 'enddate'.equals(fieldName)
                {if $mode ne 'create'}
                    {formdateinput group='meta' id='metadataEnddate' dataField='enddate' mandatory=false includeTime=trueIF useBootstrap cssClass='form-control'ENDIF}
                {else}
                ...
                {formlanguageselector group='meta' id='metadatalabel' mandatory=false __title='Choose a language' dataField='fieldName'IF useBootstrap cssClass='form-control'ENDIF}
                ELSE
                {formtextinput group='meta' id='metadatalabel' dataField='fieldName' maxLength=lengthIF useBootstrap cssClass='form-control'ENDIF}
                ENDIF
            IF useBootstrap
                </div>
            ENDIF
