...
class InstallerView {
    ...
    def private tplInit(Application it) '''
        ...
        <dl id="appName.toFirstLowerFeatureList">
            <dt>{gt text='getLeadingEntity.name.formatForDisplayCapital management.'}</dt>
            <dd>{gt text='Easy management of getLeadingEntity.nameMultiple.formatForDisplayIF getAllEntities.size > 1 and IF models.map[relations].size > 1relatedELSEotherENDIF artifactsENDIF.'}</dd>
        ...
        ENDIF
            ...
            <dd>{gt text='All parts of appName are always up to the latest version of the Zikula coreIF !targets('1.3.5') and SymfonyENDIF.'}</dd>
            <dd>{gt text='Entities, controllers, hooks, templates, plugins and more.'}</dd>
        </dl>
        <p>
            <a href="{modurl modname='appName' type='init' func='interactiveinitstepIF needsConfig2ELSE3ENDIF'}" title="{gt text='Continue'}">{gt text='Continue'}</a>
        </p>
    '''
    ...
    def private tplInitStep2(Application it) '''
        ...
        <form action="{modurl modname='appName' type='init' func='interactiveinitstep2'}" method="post" enctype="application/x-www-form-urlencoded"IF targets('1.3.5') class="z-form"ELSE class="form-horizontal" role="form"ENDIF">
            <fieldset>
                ...
                FOR modvar : getAllVariablesmodvar.tplInitStep2Var(it)ENDFOR
            </fieldset>
            <fieldset>
                ...
                <div class="IF targets('1.3.5')z-formrowELSEform-groupENDIF">
                    IF targets('1.3.5')
                        <label for="appNameActivate">{gt text='Activate appName after installation?'}</label>
                        <input id="appNameActivate" name="activate" type="checkbox" value="1" checked="checked" />
                    ELSE
                        <div class="col-lg-offset-3 col-lg-9">
                            <div class="checkbox">
                                <label>
                                    <input id="appNameActivate" name="activate" type="checkbox" value="1" checked="checked" /> {gt text='Activate appName after installation?'}
                                </label>
                            </div>
                        </div>
                    ENDIF
                ...
                IF !targets('1.3.5')
                    <div class="col-lg-offset-3 col-lg-9">
                ENDIF
                    {formbutton commandName='submit' __text='Submit' class='IF targets('1.3.5')z-bt-saveELSEbtn btn-successENDIF'}
                IF !targets('1.3.5')
                    </div>
                ENDIF
                </div>
    ...
    def private tplInitStep2Var(Variable it, Application app) '''
        <div class="IF app.targets('1.3.4')z-formrowELSEform-groupENDIF">
